<template>
	<div class="pandastudio-download-settings">
		<div class="tune-buttons">
			...
			<nvTooltip>
				<template #trigger>
					...
					@click="handleToggleAlign('center')"
					>
		...
		<div class="links tune-box border-top border-bottom">
			<div class="tune-box">
				...
				<div v-for="(link,index) in links" :key="link">
					...
					<div class="mb-3 text-right"><NButton text type="error" @click="handleRemove(index)"><i class="el-icon-delete mr-2"></i>删除</NButton></div>
				</div>
			</div>
		</div>
	</div>
...
<script>
export default {
	name: 'pandastudio-download-settings',
	components:{
...
<style>
.nv-inner-blocks .pandastudio-download-settings .need_reply {
	display: none;
}
</style>