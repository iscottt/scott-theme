  ...
  <div class="container flex items-center xxs:flex-col-reverse xs:flex-col-reverse xxs:justify-center xs:justify-center">
    <div class="metas" :class="{ 'fade-right': fadeTowards == '-', 'fade-left': fadeTowards == '+' }">
      <div class="categories">
        <template v-if="$route.params.taxonomy == 'category'"> <svg-icon name="book" /> 分类：{{ currentSlide.categories.join(' · ') }} </template>
        <template v-else-if="$route.params.taxonomy == 'tag'"> <svg-icon name="tag" /> 标签：{{ currentSlide.categories.join(' · ') }} </template>
        <template v-else>
...
.metas {
  ...
  &.fade-right {
    ...
    animation: toRight ease-in 0.38s;
  }
  &.fade-left {
    opacity: 0;
    transform: translateX(5em);
    animation: toLeft ease-in 0.38s;
  }
