<template>
	<div class="top-nav-wrapper"><div class="top-nav" :class="{
		float,
		'float-in': floatIn,
...
@media (max-width: 991.5px) {
	...
	.menu-wrapper {
		...
		width: 280px;
		background: linear-gradient(hsla(0,0%,100%,.9), hsla(213,40%,96%,.9));
		// background: linear-gradient(rgba(255,255,255,.7),rgba(240,240,240,.7));
		box-shadow: 0 0 0 1px rgba(255,255,255,.6) inset,0 0 0 1px var(--gray-opacity-1), 0 5px 8px var(--gray-opacity-2);
		border-radius: 10px;
	...
	.menu-button {
		...
		span {
			...
			margin: 2.5px 0;
			transition: .35s, margin .2s ease-out .2s,width .2s ease-in-out .1s, transform .3s;
		...
		&.active {
			span {
				...
				margin: -0.5px;
				background: #fff;
				transition: .35s, margin .2s,width .2s .2s, transform .3s ease-in-out .1s;
				&:first-child {
...
.top-nav.float {
	...
	box-shadow: 0 -1px rgba(255,255,255,.6) inset,0 1px var(--gray-opacity-1), 0 5px 8px var(--gray-opacity-1);
	animation: none;
	transition: .25s;
}
