<template>
	...
	>
		...
		<template v-for="item in menu">
			...
			@mouseenter="(event)=>{level == 0 && handleMouseenter(event)}"
			@mouseleave="(event)=>{level == 0 && handleMouseleave(event)}"
			>
			<a-link
...
@media (min-width: 992px) {
	...
	ul.menu {
		...
		> li {
			...
			&.anchor {
				...
				height: 3px;
				padding: 0 15px;
				transition: transform cubic-bezier(.3,1.6,.7,1) .5s,width cubic-bezier(.3,1.6,.7,1) .5s, opacity .5s, top .2s;
				&:before {
					content: "";
					display: block;
					width: 100%;
	...
	.float-in {
		...
		ul.submenu {
			...
			background: linear-gradient(hsla(0,0%,100%,.95),hsla(213,40%,96%,.95)) no-repeat;
